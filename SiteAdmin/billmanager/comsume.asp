...
If module="check" Then
	...
	sql="insert into countlist (u_id,u_moneysum,u_in,u_out, u_countid , c_memo ,c_date ,c_dateinput ,c_datecheck,c_check,c_type,o_id,p_proid,u_Balance) values ("&u_id&", "&u_out&", 0, "&u_out&", '"&u_countid&"' , '"&c_memo&"', '"&c_date&"','"&now()&"', '"&now()&"',0,9,0,'',"&u_Balance&")"
	conn.execute(sql)
		msg="成功"
		u_countid=""
...
<table width='100%' border='0' align='center' cellpadding='2' cellspacing='1'>
  <tr class='topbg'>
    <td height='30' align="center" ><strong>手工扣款</strong></td>
  </tr>
</table>
          ...
          <tr> 
            <td valign="top"  align="center"> 
              <%
						Response.Write msg
						%>
              <FORM action="comsume.asp" method=post name=form1>
                <table width="100%" border=0 align=center cellpadding=3 cellspacing=0 bordercolordark=#FFFFFF class="border">
                  <TR> 
                    <TD width="39%" align="right" class="tdbg">&nbsp;</TD>
                    <TD width="61%" class="tdbg">&nbsp;</TD>
                  </TR>
                  <TR> 
                    <TD width="39%" align="right" class="tdbg">凭证编号：<%=Ucase(left(session("user_name"),4))%>-</TD>
                    <TD width="61%" class="tdbg"> 
                  ...
                  <TR> 
                    <TD align="right" class="tdbg">消费扣款：</TD>
                    <TD class="tdbg"> 
                      <INPUT name=sid3 size="20"  value="<%=u_out%>">
                      元</TD>
                  </TR>
                  <TR> 
                  ...
                  <TR> 
                    ...
                    <TD class="tdbg"> 
                      <INPUT name=sid4 size="20"  value="<%=c_date%>">
                      (yyyy-mm-dd)</TD>
                  </TR>
                  <TR> 
...
<strong>手工扣款</strong>: 用户申请退款，将款从会员帐号中扣除。用户可用金额减少。<br>
可用于其他需要手动扣款的项目，比如帮用户制作了一个网页，收费300元。可以用本功能扣除。 <br>
