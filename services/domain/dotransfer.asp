...
if lcase(USEtemplate)="tpl_2016" then
	domainlist=requesta("domainlist")
	domain=""
	authcode=""
	For Each line In Split(domainlist,Chr(13))
		If InStr(line," ")>0 Then
			ipos=instr(line," ")
			If Trim(domain)	="" Then
				domain=mid(line,1,ipos)
				authcode=mid(line,ipos+1,Len(line))
			Else
				domain=domain&vbcrlf&mid(line,1,ipos)
				authcode=authcode&vbcrlf&mid(line,ipos+1,Len(line))
			End if
		End if
	Next
 else
