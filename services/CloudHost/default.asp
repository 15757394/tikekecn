...
isfull=false
if requesta("act")<>"update" then
	isup=application("services_cloudhost_sj")
	'response.write(not isdate(isup))
	if not isdate(isup) then isupdate=true:isfull=true   '无或内容错误
	if isupdate then  isup=now()  '强制更新
	'response.write(datediff("S",isup,now()))
	if datediff("S",isup,now())>7200 then isupdate=true::isfull=true
	htmlstr=application("services_cloudhost_html_"&session("u_levelid"))
	if htmlstr="" then isupdate=true
else
	isupdate=True
	isfull=true
...
die htmlstr&"<!--"&isupdate&" time:"&application("services_cloudhost_sj")&"  lev:"&session("u_levelid")&" update:"&isupdate&"-->"
