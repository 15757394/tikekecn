...
<!--table-->
       <table class="manager-table">
         	...
         	%>
                   <tr>
                   	   <td><input type="checkbox" name="d_id" value="<%=rs("d_id")%>" /></td>
                       <td>
                          ...
                          <td><%
         								If rs("isreglocal") then
											...
											if trim(alldomain)="" then
												alldomain=rs("strDomain")
											else
												alldomain=alldomain&","&rs("strDomain")
											end if
         								End If
         						%></td>
					   <td data-dm="<%=rs("strDomain")%>">
								<%If rs("isreglocal") then%>
									无
								<%
								else
...
function gouhusure()
{
	...
	location.href="mouldgouhu.asp?act=show&ids="+ids.join(",")
}
