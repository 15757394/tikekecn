...
if act="checkcode" then
	code=requesta("code")
	if trim(code&"")="" then die echojson("500","验证码为空","")
	if len(code&"")<>6 then die echojson("500","请录入6位验证码","")
	...
	<%else%>
		...
		<table class="manager-table">
			...
			<tr>
				<th>手机号:</th>
				<td align=left>
				  <input type="text" name="mobile" value="<%=msn_msg%>" maxlength=11 class="manager-input s-input" readonly>
				  <button class="manager-btn s-btn" onClick="location.href='default2.asp'">修改号码</button>  <button class="manager-btn s-btn"   id="sendauthcode">发送验证码</button> </td>
			</tr>
			<tr>
				<th>验证码:</th>
				<td align=left>
	...
	<script>
		...
		$(function(){
			...
			$("#checkauthcode").click(function(){
				...
				if(isNaN(code) || code.length!=6){ $.dialog.alert("验证码错误！");return false;}
				obj.attr("disabled",true)
