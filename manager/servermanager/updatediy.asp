...
$(function(){	
	...
	$("input[name='subbtton']:button").click(function(){
		if(Number(<%=ds.olddata%>)<=Number($("input[name='data']:range").val())){
...
function tuijian(){
	var tjArr=[{title:"原配置",content:{cpu:<%=ds.oldcpu%>,ram:<%=ds.oldram%>,data:<%=ds.olddata%>,flux:<%=ds.oldflux%>}},
				{title:"入门型",content:{cpu:2,ram:2,data:60,flux:2}},
			    {title:"标准型",content:{cpu:2,ram:3,data:80,flux:3}},
				{title:"商务型",content:{cpu:3,ram:5,data:100,flux:4}},
				{title:"舒适型",content:{cpu:3,ram:5,data:200,flux:5}},
				{title:"企业型",content:{cpu:4,ram:7,data:300,flux:8}},
				{title:"豪华型",content:{cpu:5,ram:8,data:500,flux:10}}];
...
function setbuttonrangeval(data,flux){//设置有加减号的range
	$("#data_range").html("<input type=\"range\" name=\"data\" min=\"50\" max=\"1000\" step=\"10\" value=\""+data+"\" /><div style=\"float:left; margin-top:7px\">GB</div>");
	
	//$("input[name='data']:range").eq(0).val(data);
...
function getprice(){	
	...
	if(data<<%=ds.olddata%>){
		...
		setbuttonrangeval(<%=ds.olddata%>,flux);
		data=<%=ds.olddata%>;
	...
	var info="id=<%=ds.aid%>&act=getprice&cpu="+ cpu +"&ram="+ ram +"&flux="+ flux +"&data="+ data +"&room="+ room +"&blday="+ blday +"&servicetype="+ escape(servicetype);
//	document.write(info);
          ...
          <div class="z_buymsg clearfix" style="padding:20px 0 0 0; ">
            <dl class="linebox1" id="upbox" style="width:700px;">
              ...
              <dd class="clearfix" id="datamsg" style="margin-bottom:30px;" >
                ...
                <label class="data_label">硬盘<span ><img src="/images/CloudHost/ebscloud/help.jpg" onMouseOver="javascript:seetitle(this,'免费提供50G硬盘，其中系统盘分配大小为30GB；数据盘默认为20GB，数据盘以10G递增。',300,32)" onMouseOut="javascript:closetitlebox()"  style="cursor:pointer; margin-left:3px;" /></span></label>
                ...
                <div style="clear:both; padding-left:150px;margin-top: 50px" class="clearfix"> 系统盘分配大小为30GB </div>
                </label>
              </dd>
