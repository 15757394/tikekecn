...
'删除10分钟内未开通主机
conn.execute "delete from HostRental where datediff("&PE_DatePart_S&",submittime,"&PE_Now&")>=600 and (AllocateIP is null or AllocateIP='') and Start="&PE_False&""
...
%>
        <tr align=middle bgcolor="<%=tdcolor%>">
		 ...
		 %></label>&nbsp;&nbsp;<img src="/images/1341453609_help.gif" onclick="setbname(<%=rs("id")%>,'<%=rs("s_bz")%>')" /></td>
          <td   class="tdbg"><%=Rs("AllocateIP")%>&nbsp;</td>
...
%>		  </td>
	  ...
	  <a href="renew.asp?id=<%=rs("id")%>">续费</a> 
		...
		if Rs("HostID")="ebscloud" then
		%>
         <a href="updatediy.asp?id=<%=rs("id")%>">升级</a>
        <%
		else
		%>
