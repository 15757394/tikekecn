...
function server_renew()
	...
	if success(loadRet) then
		if not consume(u_name,needprice,false,"server-" & Left(Cstr(Clng(rnd()*100000)),6) , "独立IP主机续费" & allocateip ,sb.p_proid,"") then
			result="500 err"
...
function vhost_renew()
	...
	if isip(otherip) then
		...
		sslprice=0
		sqlstring="select top 1 s_ssl from vhhostlist where S_ownerid=" &  u_id & " and s_comment='" & s_comment &" '"
		set chkrs=conn.execute(sqlstring)
		if not chkrs.eof then
			s_ssl=chkrs("s_ssl")
			if not isnumeric(s_ssl&"") then s_ssl=0
			if s_ssl>0 then sslprice=getneedprice(u_name,"vhostssl",s_year,"renew")
