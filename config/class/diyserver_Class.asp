...
Class diyserver_class
	...
	dim cpu,ram,data,flux,room,PayMethod,renewTime,servicetype,CHOICE_OS,addServerPrice,aid,hostStart,paytype,buycount,disktype
	dim oldcpu,oldram,olddata,oldflux,oldroom,oldservicetype,oldCHOICE_OS,oldaddServerPrice,oldcpuhz,oldramsize,oldexpiredate,h_oldprice,oldMoneyPerMonth,oldhostStart
	dim cpuhz,ramsize,sstate,isUPGRADE
	dim appObj,MoneyPerMonth,monthnum,vpsIP,vpsPassWord,RamdomPass,Maxwhiteline,starttime,differentstr,isCloud,blday
	dim errstr,isfast,VPSOpen,blprice,bldayArr,souxvfei,all_day,leavings_day,money_daycj,newprice,oldprice,oldserverip,oldhostid
	...
	dim newroom,isupdate,MainBoardStr,MainBoard,jsondb
				...
				if app_item<>"" then
					...
					for each str_item in split(appcontent,";")
						if str_item<>"" then
							...
							c4=trim(strarr(12))		
							d=	trim(strarr(13))	
							if d=0 or IsOldSeting(apptitle,a) then
								appObj(apptitle).item(a)=array(a1,a2,a3,a4,b1,b2,b3,b4,c1,c2,c3,c4)
							end if
						end if
					next
					set appObj_=nothing
...
'							 "fax:"& mobile & vbCrLf & _
	...
	public function setParam(byval pay_Item)
		...
		mustinput="p_proid,Company,Name,u_trade,Telephone,Email,QQ,Fax,Zip,Address,HostID,cpuhz,cpu,data,ramsize,ram,flux,Years,Memo,CHOICE_OS,checkStup,servicetype,PayMethod,renewtime,Room,Apply,MainBoard,isvps,isnewtype,aid,paytype,disktype"
...
public function upToManager(byval needMoney)
	...
	public function getprice(byval dotype)
		...
		realPrice=cdbl(cpu_price) + cdbl(ram_price) + cdbl(flux_price) + cdbl(data_price) + cdbl(addServerPrice)
		...
		fullprice=round(cdbl(realPrice) * cdbl(renewTime),2)
		if paytype=1 then
			result=paytestPrice
			monthnum=0
		else
			result=round(cdbl(paymoney) * cdbl(renewTime),2)'vps价格*购买时间+总月份*服务价格
		end if
	...
	function getfirstprice()
		...
		fluxmaxprice=cdbl(appObj("flux")(room)(8)):if fluxmaxprice=0 then fluxmaxprice=fluxprice
		...
		if disktype="ssd" then 
			data_price=data_price * 3
			if monthnum=12 then
				data_price=data_price+300
			else
				data_price=data_price+30 * monthnum
			end if
		end if
	...
	function getrealprice(byval price,byval jiprice,byval banprice,byval yearprice,byval monthnum_)
		result=0
		if price=0 or price="" then price=99999
...
'		end if
	...
	if application("server_diy_config_sj")="" or application("server_diy_config")="" then
		application("server_diy_config")="{"&trim(GetRemoteUrl("http://api.west263.com/api/Agent_API/getdiyconfig.asp"))&",""now"":"""&now()&"""}"
...
'			returnJson=regReplace(application("server_diy_config"),"\""paytestprice\"":\""(\d+)\""","""paytestprice"":"""&diypaytestPrice&"""")
'		else
'			returnJson=application("server_diy_config_sj")
'		end if
		'die returnJson
	end function
